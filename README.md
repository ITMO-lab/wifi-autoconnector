...
[**2 Исследование возможностей демонов в Linux**  **6**](#2--исследование-возможностей-демонов-в-linux)

[**3 Исследование способов синхронизации топиков в ROS**  **7**](#3--исследование-способов-синхронизации-топиков-в-ros)

  [3.1 Решения на сокетах  7](#31--решения-на-сокетах)

  [3.2 Пакет multimaster_fkie для ROS  7](#32--пакет-multimaster_fkie-для-ros)

[**4 Разработанная программа**  **8**](#4--разработанная-программа)

  [4.1 Описание  8](#41--описание)

  [4.2 Решатели  10](#42--решатели)

    [4.2.1 Stairs  10](#421--stairs)

[**5 Процесс установки и настройки**  **12**](#5--процесс-установки-и-настройки)

  [5.1 Установка ubuntu 16.04 на raspberry pi 4  12](#51--установка-ubuntu-1604-на-raspberry-pi-4)

  [5.2 Установка NetworkManager и pynmcli  13](#52--установка-networkmanager-и-pynmcli)

  [5.3 Установка пакета проекта качестве фонового демона  14](#53--установка-пакета-проекта-качестве-фонового-демона)

    [5.3.1 Установка  14](#531--установка)

    [5.3.2 Настройки  15](#532--настройки)

[**6 Поиск Wi-Fi роутера для увеличения дальности и уровня безопасности связи**  **16**](#6--поиск-wi-fi-роутера-для-увеличения-дальности-и-уровня-безопасности-связи)

[**7 Поиск антенн для роутера и анализ способов их размещения в роутере Xiaomi Mi WiFi Router pro (r3p)**  **20**](#7--поиск-антенн-для-роутера-и-анализ-способов-их-размещения-в-роутере-xiaomi-mi-wifi-router-pro-r3p)

[**8 Тесты**  **23**](#8--тесты)

  [8.1 Анализ скорости соединения через Wi-Fi сеть  23](#81--анализ-скорости-соединения-через-wi-fi-сеть)

  [8.2 Анализ возможности доступа к ROS через внешний IP адрес  24](#82--анализ-возможности-доступа-к-ros-через-внешний-ip-адрес)

    [8.2.1 Установка и настройка сервиса VPN Hamachi на raspberry pi 4  24](#821--установка-и-настройка-сервиса-vpn-hamachi-на-raspberry-pi-4)

    [8.2.2 Тест скорости при подключении системы через открытый VPN сервис Hamachi.  25](#822--тест-скорости-при-подключении-системы-через-открытый-vpn-сервис-hamachi)

[**9 Результат**  **26**](#9--результат)
...
Wi-Fi-Direct - хранит огромное множество уязвимостей в безопасностеи. Простейшая доступнка атака - замена MAC адреса и перехват подключения устройства из-за более высокого уровня сигнала.
...
а)  Аутентификатор передает EAP-запрос на клиентское устройство, как только обнаруживает активное соединение.

б)  Клиент отправляет EAP-ответ - пакет идентификации. Аутентификатор пересылает этот пакет на сервер аутентификации (RADIUS).

в)  RADIUS проверяет пакет и право доступа клиентского устройства по базе данных пользователя или другим признакам и затем отправляет на аутентификатор разрешение или запрет на подключение. Соответственно, аутентификатор разрешает или запрещает доступ в сеть.
...
a)  None - отсутствие шифрования, данные передаются в открытом виде

б)  WEP - основанный на алгоритме RC4 шифр с разной длиной статического или динамического ключа (64 или 128 бит)

в)  CKIP - проприетарная замена WEP от Cisco, ранний вариант TKIP

г)  TKIP - улучшенная замена WEP с дополнительными проверками и защитой

д)  AES/CCMP - наиболее совершенный алгоритм, основанный на AES256 с дополнительными проверками и защитой

Доступные в WPA2-Enterprise способы проверки подлинности подключаемого устройства внешним сервером:

а)  EAP-FAST (Flexible Authentication via Secure Tunneling) - разработан фирмой Cisco; позволяет проводить авторизацию по логину-паролю, передаваемому внутри TLS туннеля между суппликантом и RADIUS-сервером

б)  EAP-TLS (Transport Layer Security). Использует инфраструктуру открытых ключей (PKI) для авторизации клиента и сервера (суппликанта и RADIUS-сервера) через сертификаты, выписанные доверенным удостоверяющим центром (CA). Требует выписывания и установки клиентских сертификатов на каждое беспроводное устройство, поэтому подходит только для управляемой корпоративной среды. Сервер сертификатов Windows имеет средства, позволяющие клиенту самостоятельно генерировать себе сертификат, если клиент - член домена. Блокирование клиента легко производится отзывом его сертификата (либо через учетные записи).

в)  EAP-TTLS (Tunneled Transport Layer Security) аналогичен EAP-TLS, но при создании туннеля не требуется клиентский сертификат. В таком туннеле, аналогичном SSL-соединению браузера, производится дополнительная авторизация (по паролю или как-то ещё).

г)  PEAP-MSCHAPv2 (Protected EAP) - схож с EAP-TTLS в плане изначального установления шифрованного TLS туннеля между клиентом и сервером, требующего серверного сертификата. В дальнейшем в таком туннеле происходит авторизация по известному протоколу MSCHAPv2

д)  PEAP-GTC (Generic Token Card) - аналогично предыдущему, но требует карт одноразовых паролей (и соответствующей инфраструктуры)
...
[https://ubiquity-pi-image.sfo2.cdn.digitaloceanspaces.com/2020-02-10-ubiquity-xenial-lxde-raspberry-pi.img.xz
  ](https://ubiquity-pi-image.sfo2.cdn.digitaloceanspaces.com/2020-02-10-ubiquity-xenial-lxde-raspberry-pi.img.xz)Также можно поискать более актуальную версию на сайте:
...
cd /etc/systemd/system sudo git clone \ https://github.com/ITMO-lab/wifi-autoconnector.git sudo cp wifi-autoconnector/wifi-autoconnector.service . sudo systemctl daemon-reload sudo systemctl stop wifi-autoconnector.service sudo systemctl disable wifi-autoconnector.service sudo systemctl enable wifi-autoconnector.service sudo systemctl start wifi-autoconnector.service sudo systemctl status wifi-autoconnector.service

Сервис периодически ест ресурсы процессора, поэтому его можно выключать, когда он не нужен, командой:

cd /etc/systemd/system sudo systemctl stop wifi-autoconnector.service

Или обратно включать командой:

cd /etc/systemd/system sudo systemctl start wifi-autoconnector.service
...
{ . . .  "{BSSID} or {SSID}": {   "password": "{your password}",   "login": "{your WPA2-Enterprise login}"  }, . . . }
...
roscore  rosrun fkie_master_discovery zeroconf rosrun fkie_master_sync master_sync
...
update_min_signal = 20 # from 0 to 100  update_max_signal = 40 # from 0 to 100
...
ignore_wrong_bssid_timeout = 0.0 ignore_wrong_auth_timeout = 30.0 ignore_unexpected_error_timeout = 60.0
...
а)  Поддержка системы контроля пользователей RADIUS (возможность работы с ней или предустановленная система контроля версий).

б)  Поддержка установки OpenWRT (см пункт 1.2) с целью усиления безопасности, поддержки таких важных библиотек как OpenSSL в актуальном состоянии, а также возможности установки RADIUS сервера на роутер вручную.

в)  Поддержка роутером, как минимум, стандартов WPA2-Enterprise с шифрованием AES/CCMP и фреймворком аутентификации EAP-TLS, PEAP-MSCHAPv2 или PEAP-GTC с.

  Рассмотренные модели с их кратким описанием:

а)  XIAOMI Mi WiFi Router [3g v.2]
...
д)  Xiaomi Mi WiFi Router pro (r3p)
...
Для обеспечения водонепроницаемости роутера можно использовать антенны с водонепроницаемым дизайном.

(Например: https://clck.ru/MT6Um или https://clck.ru/MT6XJ)
...
sudo dpkg -i logmein-hamachi_*_armhf.deb /etc/init.d/logmein-hamachi start sudo systemctl daemon-reload sudo systemctl enable logmein-hamachi.service sudo systemctl start logmein-hamachi.service
...
sudo hamachi logon sudo hamachi attach <Email адрес аккаунта> sudo hamachi **do**-join <network ID> sudo hamachi list
